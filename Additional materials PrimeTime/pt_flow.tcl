...
foreach_in_collection group [remove_from_collection [get_path_groups] $default_groups] {
  ...
  if { [sizeof_collection [get_timing_paths -group $group_name]] } {
    redirect -append -file ./reports/${DESIGN_NAME}.timing_setup.rpt {
      report_timing \
                -nosplit \
                -capacitance \
                -transition_time \
...
foreach_in_collection group [remove_from_collection [get_path_groups] $default_groups]  {
  set group_name [get_object_name $group]
  if { [sizeof_collection [get_timing_paths -group $group_name]] } {
    redirect -append -file ./reports/${DESIGN_NAME}.timing_hold.rpt {
      report_timing \
                -delay_type min \
                -nosplit \
                -capacitance \
...
set_operating_conditions -analysis_type on_chip_variation -max tt0p8v25c -min tt0p8v25c 
###############################
...
#set_noise_lib_pin [get_pins -filter "direction==out"] [get_lib_pins */BUFFD2BWP20P90/I]
#set_noise_lib_pin [get_pins -filter "direction==in"] [get_lib_pins */BUFFD2BWP20P90/Z]
